...
- Example:  we test people for COVID using an antigen test on a nasal swab, a PCR test on a throat swab, and the same antigen test on the same throat swab
  ...
  * The virus may be present in the throat, or nose, or neither, or both
  * If it is only present in the throat then only the PCR and second antigen test should test positive
  * On the other hand, we use the same antigen test twice, so it might cross-react with the same non-target virus
...
```{r}
# Other parameter values to simulate:
N <- 20000
...
cat(readLines('covidmodel.txt')[-(1:2)], sep='\n')
```
...
I have omitted the trace plots here but make sure to always check them!
...
The posterior estimates for sensitivity have been affected for both tests:
...
The effective sample size is much lower, because the model is less identifiable.  But otherwise the model does a reasonable job of estimating the parameters due to the large sample size.  This might not be the case with a smaller sample size!
