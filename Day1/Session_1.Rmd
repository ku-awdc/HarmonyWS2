...
The true `Status` is always the same as the result of `Test1`
. . .
...
- Copy and paste the following R code given above into this R script file:
...
The prevalence estimate and 95% CI from the frequentist method is `r coef(freq_model)` (`r plogis(confint(freq_model))[1]` - `r plogis(confint(freq_model))[2]`).  The equivalent estimates from the Bayesian method are:
...
Notice that the median estimate for the Bayesian model has shifted from `r med1` to `r med2`. This is due to the relatively larger impact of priors with smaller datasets.  All Bayesian models *must* include priors - even when you don't want to. We will return to this concept later today.
...
Convergence is OK and the sample size is sufficient for all parameters.

The posterior for `observed_prevalence` is the same as for `prevalence` in the old model.  But the posterios for the new (true) `prevalence`, `sensitivity` and `specificity` are extremely wide!  The model is only able to identify the `observed_prevalence`, and not the other three parameters.  There is simply not enough information to disentangle them - persuade yourself this is true by considering the following possibilities:
